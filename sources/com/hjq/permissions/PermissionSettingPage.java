...
final class PermissionSettingPage {
    ...
    static Intent getSmartPermissionIntent(Context context, List<String> list) {
        ...
        if (list.size() == 1) {
            ...
            return getApplicationDetailsIntent(context);
        } else if (!PermissionUtils.isAndroid11() || list.size() != 3 || !list.contains(Permission.MANAGE_EXTERNAL_STORAGE) || !list.contains(Permission.READ_EXTERNAL_STORAGE) || !list.contains(Permission.WRITE_EXTERNAL_STORAGE)) {
            return getApplicationDetailsIntent(context);
        } else {
            return getStoragePermissionIntent(context);
        }
