...
public abstract class BleScanPresenter implements BluetoothAdapter.LeScanCallback {
    ...
    private void correctDeviceAndNextStep(final BleDevice bleDevice) {
        if (this.mNeedConnect) {
            BleLog.m1844i("devices detected  ------  name:" + bleDevice.getName() + "  mac:" + bleDevice.getMac() + "  Rssi:" + bleDevice.getRssi() + "  scanRecord:" + HexUtil.formatHexString(bleDevice.getScanRecord()));
        ...
        if (!atomicBoolean.get()) {
            BleLog.m1844i("device detected  ------  name: " + bleDevice.getName() + "  mac: " + bleDevice.getMac() + "  Rssi: " + bleDevice.getRssi() + "  scanRecord: " + HexUtil.formatHexString(bleDevice.getScanRecord(), true));
            this.mBleDeviceList.add(bleDevice);
            this.mMainHandler.post(new Runnable() {
                public void run() {
